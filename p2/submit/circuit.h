...
class Circuit
{
    ...
    public:
        ...
        int createMUX4Node(Node* select1, Node* select2,
                ...
                Node* output);
        int createMUX2Node(Node* select, Node* input1, Node* input2, Node* output);

        int createNOT1Node(Node* input, Node* output);
        int createFullAdder(Node* input1, Node* input2, Node* cin,
                Node* output, Node* cout);


        // start module definitions
        int createADDModule(const string &input1, const string &input2,
                const string &cin,
                const string &output, const string &cout, unsigned int numBits);

        int createSUBModule(const string &input1, const string &input2,
                const string &output, unsigned int numBits);

        int createSHIFTModule(const string &input,
                const string &output, unsigned int numBits, unsigned int numShift);

        int arithmeticExtend(const string &input,
                unsigned int numBits, unsigned int extendBits);

        int createMINModule(const string &input1, const string &input2,
                const string &output, unsigned int numBits);

        int createABSModule(const string &input, const string &output,
